...
try:
    import meta_prompt_agent.core.agent as agent_logic
    from meta_prompt_agent.prompts.templates import STRUCTURED_PROMPT_TEMPLATES
    from meta_prompt_agent.config.settings import OLLAMA_MODEL, OLLAMA_API_URL 
except ImportError as e:
    st.error(f"启动错误: {e}. 请确保 agent_logic.py 和 prompt_templates.py 文件在同一目录下。")
    st.stop() 

from meta_prompt_agent.config.settings import (
    OLLAMA_MODEL,
    OLLAMA_API_URL
)

from meta_prompt_agent.config.logging_config import setup_logging
import logging # 仍然需要导入logging来获取logger实例

setup_logging() # 你可以传递不同的级别，如 setup_logging(logging.DEBUG)

# 获取logger实例 (现在它会使用我们刚刚设置的全局配置)
...
def main():
    try: # <--- 包裹主要的UI渲染和逻辑
        ...
        st.caption(f"使用 Ollama 模型: {OLLAMA_MODEL} | API: {OLLAMA_API_URL}")
        ...
        if st.button("🚀 生成优化提示词", key="btn_generate", type="primary", use_container_width=True):
            try: # <--- 包裹按钮点击后的核心处理逻辑
                ...
                if selected_template_name != "无": # If a structured template is selected
                    ...
                    if template_vars_needed: # Only check if template actually has variables defined
                        for var_name in template_vars_needed:
                            if not structured_vars_input.get(var_name, "").strip():
                                ...
                                logger.warning(f"用户输入验证失败：模板 '{selected_template_name}' 缺少变量 '{var_name}'。")
                    ...
                    logger.warning("用户输入验证失败：初步请求和模板变量均为空。")
                ...
                logger.exception("在处理“生成优化提示词”按钮点击时发生未捕获的UI层错误。")
                st.error("应用发生内部错误，请刷新页面或联系支持。")
        ...
        if st.session_state.processing_results:
            ...
            if results.get("error_message"):
                ...
                if results.get("error_details"):
                    ...
                    # 对于普通用户，可能不需要显示详细的 error_details JSON。
                    # 可以记录到日志中，或者只在某种“开发者模式”下显示。
                    logger.error(f"处理请求时发生错误。用户看到的错误: '{results['error_message']}'. 详细错误对象: {results['error_details']}")
            else:
                    ...
                    if st.button("提交反馈", key="btn_submit_feedback_tasks"):
                        try:
                            ...
                            if agent_logic.save_feedback(current_feedback_list):
                                ...
                                st.error("保存反馈失败。请检查后台日志或联系管理员。")
                                logger.error("保存用户反馈失败。agent.save_feedback() 返回 False。") # 确保有日志记录
                        except Exception as e:
                            st.error("保存反馈时发生内部错误，请稍后再试。")
                            logger.exception("在处理“提交反馈”按钮点击时发生未捕获的UI层错误。")
                else:
        ...
        logger.exception("Streamlit 应用主流程发生未捕获的严重错误。")
        # 对于最外层的错误，可能很难优雅地在UI上显示，
        # 因为Streamlit的渲染可能已经中断。
        # 但至少日志中会有记录。
        # 可以尝试显示一个非常简单的错误信息，如果Streamlit还允许的话。
        st.error("应用程序遇到严重问题，可能需要重启。请查看日志了解详情。")
